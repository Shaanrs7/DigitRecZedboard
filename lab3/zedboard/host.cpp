     ...
     for (int i=0; i<N; i++)
		{
			 bit64_t test_i;
			 digit temp = inputs[i];
 			 test_i(temp.length()-1, 0) = temp(temp.length()-1,0);

			 int64_t input_t = test_i; 
	...
	for (int i=0; i<N; i++)
		{
			 ...
			 bit32_t recvd_temp = recvd_i;
		 	 interpreted_digit(interpreted_digit.length()-1,0) = recvd_temp(interpreted_digit.length()-1,0);
		         num_test_insts=num_test_insts+1;	
			 if (expecteds[i]!= (int)interpreted_digit)
			 {
				error= error+1;
			 }
